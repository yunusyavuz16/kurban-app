...
export default function KurbanOrderManager() {
  ...
  const reorderMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      alert("Sıralama güncellenirken hata oluştu: " + (error?.response?.data?.error || error.message));
    }
  });
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>, targetAnimal: Animal) => {
    e.preventDefault();
  ...
  return (
    <div className="!bgwhite rounded-xl shadow-lg p-4 sm:p-6 border border-gray-200">
      <h2 className="text-lg sm:text-xl font-semibold text-gray-800 mb-2 sm:mb-4">Kurban Sıralaması (Beklemede)</h2>
        ...
        {waitingAnimals?.map((animal) => (
          ...
          >
            <div className="flex items-center space-x-3 sm:space-x-4">
              <span className="text-base sm:text-lg font-semibold text-gray-900 w-12 text-right">
                #{animal.order_number}
              </span>
