...
export default function AdminPanel() {
  ...
  const [activeTab, setActiveTab] = useState<
    "orderManager" | "userManagement" | "statusManagement"
  >("orderManager");
  ...
  const statusMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      setStatusError(
        error?.response?.data?.error || "Durum kaydedilirken bir hata oluştu."
      );
    },
  });
  ...
  const deleteStatusMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      alert(
        error?.response?.data?.error || "Durum silinirken bir hata oluştu."
      );
    },
  });
  ...
  const handleDeleteStatus = (statusId: string) => {
    if (
      window.confirm(
        "Bu durumu silmek istediğinize emin misiniz? Bu durumu kullanan kurbanlar varsa işlem başarısız olacaktır."
      )
    ) {
  ...
  return (
    <div className="min-h-screen !bg-gray-50 p-4 sm:p-8">
      ...
      <div className="mb-4 flex flex-row space-x-2">
        ...
        <button
          onClick={() => setActiveTab("statusManagement")}
          className={`px-4 py-2 ${
      ...
      {activeTab === "orderManager" && (
        <div className="mb-6 sm:mb-8 bg-white">
          <KurbanOrderManager />
        </div>
      )}
      {activeTab === "userManagement" && (
            ...
            {kurbanStatuses?.map((status) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <span
                    className={`px-3 py-1 rounded-lg text-xs sm:text-sm font-semibold border ${status.color_bg} ${status.color_text} ${status.color_border}`}
                  >
                    {status.label}
                  </span>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    ...
                    disabled={
                      ...
                      deleteStatusMutation.variables === status.id
                    }
                  >
                    Sil
                  </button>
            ...
            <form onSubmit={handleStatusSubmit} className="space-y-4">
              ...
              <div className="mt-6 flex flex-col sm:flex-row sm:justify-end sm:space-x-3 space-y-3 sm:space-y-0">
                <button
                  ...
                  className={`w-full sm:w-auto inline-flex justify-center rounded-md border border-transparent px-4 py-2 text-sm font-medium text-white shadow-sm ${
                ...
                <button
                  ...
                  className="  w-full sm:w-auto inline-flex justify-center rounded-md border !border-gray-500 !bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:!bg-gray-50"
                >
                  İptal
                </button>
              </div>
