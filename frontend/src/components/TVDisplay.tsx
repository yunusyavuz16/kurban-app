...
export default function TVDisplay() {
  ...
  useEffect(() => {
    const eventSource = new EventSource(
      `${import.meta.env.VITE_API_URL}/kurban/subscribe`
    );
  ...
  const processingAnimals = animals
    ?.filter((animal) => animal?.status?.name === "slaughtering")
  ...
  const animalsByStatusName = animals?.reduce((acc, animal) => {
    if (
      !animal?.status?.name ||
      ["waiting", "cancelled", "done"].includes(animal.status.name)
    )
      return acc;
  ...
  const sortedStatuses = kurbanStatuses
    ?.filter((el) => !["waiting", "cancelled", "done"].includes(el?.name))
      ...
      {activeTab === "current" ? (
        <div className="space-y-6 md:space-y-8">
          <div className="!bg-white rounded-xl shadow-lg p-4 sm:p-6 md:p-8 border border-gray-200">
            ...
            <div className="grid gap-4 md:gap-6">
              {processingAnimals?.map((animal) => (
                <div
                  key={animal.id}
                  className={`p-4 sm:p-6 rounded-lg border-2 ${getStatusStyleString(
                    animal.status
                  ...
                  <div className="flex justify-between items-center">
                    <div>
                      <span className="text-2xl sm:text-3xl md:text-4xl font-bold">
                        Kurban #{animal.no} - Sıra #{animal.order_number}
                      </span>
          ...
          <div className="!bg-white rounded-xl shadow-lg p-4 sm:p-6 md:p-8 border border-gray-200">
            ...
            <div className="flex flex-col flex-wrap gap-2 sm:gap-3">
              {waitingAnimals?.map((animal) => {
                const waitingStatus = getStatusDetails("waiting");
                return (
                  <div key={animal.id} className="flex items-center justify-centerü space-x-2">
                    <span>#{animal.order_number}</span>
                    <span
                      className={`px-4 py-2 sm:px-6 sm:py-3 rounded-lg border font-medium text-base sm:text-lg md:text-xl ${getStatusStyleString(
                        waitingStatus
                      )}`}
                    >
                      Kurban #{animal.no}
            ...
            <div key={status.id} className="space-y-3 sm:space-y-4">
              ...
              <div className="space-y-2 sm:space-y-3">
                {animalsByStatusName?.[status.name]?.map((animal) => (
                  <div
                    key={animal.id}
                    className={`p-3 sm:p-4 rounded-lg border-2 ${getStatusStyleString(
                      animal.status
                    )} transition-all duration-200`}
                  >
                    <div className="flex justify-between items-center">
                ...
                {(!animalsByStatusName?.[status.name] ||
                  animalsByStatusName[status.name].length === 0) && (
